...
int q2(char *datainicial, char *datafinal, int *qtdDias, int *qtdMeses, int *qtdAnos){
    ...
    *qtdAnos = DataFin[2] - DataIni[2];
    ...
    if ( DataIni[1] == DataFin[1] && DataIni[0] > DataFin[0] )
        return 1;
...
int q4( unsigned char *strTexto, unsigned char *strBusca, int posicoes[]){
    int qtdOcorrencias = -1;
    int i,j,tam;
    int aux = 0;
